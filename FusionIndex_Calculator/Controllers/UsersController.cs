using FusionIndex_Calculator.RequestModels;
...
namespace FusionIndex_Calculator.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> Login([FromBody] ClientLoginModel model)
        {
            ...
            var res = await _loginClient.Request(new { model.Email, model.Password });

            if (res.Token == null)
            {
                await _endpoint.Publish<LogRequest>(new
                {
                    LogMessage = $"Unsuccessful login attempt of {model.Email} with IP {ip}",
        ...
        public async Task<IActionResult> RegisterNewUser([FromBody] RegisterModel model)
        {
            ...
            var res = await _registerClient.Request(new
            {
                model.Title,
                model.Firstname,
                model.Lastname,
